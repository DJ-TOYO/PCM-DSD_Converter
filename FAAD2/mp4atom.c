                                       ...
                                       const int8_t c, const int8_t d)
{
    ...
    if (mp4ff_atom_compare(a,b,c,d, 'e','d','t','s'))
        ...
        return ATOM_PODCAST;
    else if (mp4ff_atom_compare(a,b,c,d, 'a','l','a','c'))//added by Kobarin
        return ATOM_ALAC;                                 //
    else if (mp4ff_atom_compare(a,b,c,d, 'c','p','r','t'))//added by Kobarin
        return ATOM_CPRT;                                 //
    else
        return ATOM_UNKNOWN;
}
...
static int32_t mp4ff_read_alac(mp4ff_t *f, uint64_t size)
{
    uint32_t flags;
    uint8_t *config; //ALACSpecificConfig の生データ（ビッグエンディアン）
    int config_size = size - 4;//4==sizeof(flags)  24 or 48bytes
    if(config_size > 24)config_size = 48;
    else config_size = 24;
    config = (uint8_t*)malloc(config_size);
    ZeroMemory(config, config_size);
    flags = mp4ff_read_int32(f);
    mp4ff_read_data(f, config, config_size);//be=>le 変換は後で ALACDecoder::Init 内部で行う
    f->track[f->total_tracks - 1]->decoderConfigLen = config_size;
    if (f->track[f->total_tracks - 1]->decoderConfig)
        free(f->track[f->total_tracks - 1]->decoderConfig);
    f->track[f->total_tracks - 1]->decoderConfig = config;
...
static int32_t mp4ff_read_mp4a(mp4ff_t *f)
{
    ...
    else if(atom_type == ATOM_ALAC){// added by Kobarin
        size -= header_size;
        mp4ff_read_alac(f, size);
    }
    return 0;
}
...
static int32_t mp4ff_read_stsd(mp4ff_t *f)
{
    ...
    for (i = 0; i < f->track[f->total_tracks - 1]->stsd_entry_count && !f->stream->read_error; i++) /* CVE-2017-9253 */
    {
        ...
        if (atom_type == ATOM_MP4A)
        {
            ...
            f->track[f->total_tracks - 1]->type = TRACK_AUDIO_ALAC; //added by Kobarin
            mp4ff_read_mp4a(f);                                     //added by Kobarin
        } else {
