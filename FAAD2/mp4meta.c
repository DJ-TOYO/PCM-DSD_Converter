...
{
	char temp[32];
// C4996‘Î‰ž    sprintf(temp, "%.5u of %.5u", track, totalTracks);
    sprintf_s(temp, sizeof(temp), "%.5u of %.5u", track, totalTracks);
	*str = strdup(temp);
    return 0;
}
...
static int32_t mp4ff_set_metadata_name(mp4ff_t *f, const uint8_t atom_type, char **name)
{
    ...
    case ATOM_CPRT: tag_idx = 31;break;// added by Kobarin (copyright ‘Î‰ž)
			...
			if (atom_type == ATOM_DATA)
			{
				...
				if (parent_atom_type == ATOM_GENRE2 || parent_atom_type == ATOM_TEMPO)
				{
					if (subsize - header_size >= 8 + 2)
					{
						...
						{
							char temp[16];
// C4996‘Î‰ž				sprintf(temp, "%.5u BPM", val);
							sprintf_s(temp, sizeof(temp), "%.5u BPM", val);
...
// C4996‘Î‰ž			sprintf(temp,"%d",index);
						sprintf_s(temp,sizeof(temp),"%d",index);
						mp4ff_tag_add_field(&(f->tags), parent_atom_type == ATOM_TRACK ? "track" : "disc", temp, -1);
						if (total>0)
						{
// C4996‘Î‰ž				sprintf(temp,"%d",total);
							sprintf_s(temp,sizeof(temp),"%d",total);
							mp4ff_tag_add_field(&(f->tags), parent_atom_type == ATOM_TRACK ? "totaltracks" : "totaldiscs", temp, -1);
						}
						done = 1;
					}
                } else if(parent_atom_type == ATOM_COVER){//added by Kobarin
...
int32_t mp4ff_meta_find_by_name_added_by_kobarin(const mp4ff_t *f, const char *item, const char **value)
{
    ...
    uint32_t i;
    //const mp4ff_t *f = (const mp4ff_t*)vf;
    for (i = 0; i < f->tags.count; i++)
    {
        if (!stricmp(f->tags.tags[i].item, item))
        {
			*value = f->tags.tags[i].value;
            return 1;
        }
    }
...
int32_t mp4ff_meta_get_copyright(const mp4ff_t *f, char **value)
{
    return mp4ff_meta_find_by_name(f, "copyright", value);
}
